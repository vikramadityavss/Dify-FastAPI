...
def fetch_entity_and_nav_info(
    ...
    try:
        ...
        threshold_query = supabase.table("threshold_configuration").select("warninglevel").eq("thresholdtype", "USDPBDEPOSIT").eq("currency_code", "USD")
        ...
        currency_config_q = supabase.table("currency_configuration").select("*").or_(
            f"currency_code.eq.{exposure_currency},proxycurrency.eq.{exposure_currency},basecurrency.eq.{exposure_currency}"
        )
...
def structured_response(
    ...
    # USD PB Check - handle multiple possible field names
    total_usd_pb = 0
    for row in total_usd_pb_deposits_rows:
        amount = row.get("amount") or row.get("deposit_amount") or row.get("notional_amount") or 0
        try:
